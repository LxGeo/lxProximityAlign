...
namespace LxGeo
{
	...
	namespace lxProximityAlign
	{
		...
		std::vector<Boost_Polygon_2> alignmentNeighbourProxAware(std::map<std::string, matrix>& matrices_map, RasterIO& ref_raster, std::vector<Geometries_with_attributes<Boost_Polygon_2>>& input_polygons) {
			...
			std::transform(translated_points.begin(), translated_points.end(),
				std::back_inserter(translated_sup_pixels),
				[&ref_raster](auto& pt)->auto{return ref_raster.get_pixel_coords(pt); }
			);
			// Read proximity triplet
			std::vector<ProximityTriplet> translated_proximity_triplets; translated_proximity_triplets.reserve(translated_sup_pixels.size());
			std::transform(translated_sup_pixels.begin(), translated_sup_pixels.end(), std::back_inserter(translated_proximity_triplets), [&PTL](auto& px)->ProximityTriplet {return PTL.readTripletAt(px); });
			std::vector<double> support_pts_weights; support_pts_weights.reserve(translated_sup_pixels.size());
			auto before_after_prox_weight = [](double p_b, double p_a)->double { return double(1) - ((p_a - p_b) / (1 + p_a + p_b)); };
			for (size_t c_pt_idx = 0; c_pt_idx < translated_sup_pixels.size();++c_pt_idx) 
				support_pts_weights.push_back(before_after_prox_weight(proximity_triplets[c_pt_idx].prox_value, translated_proximity_triplets[c_pt_idx].prox_value));

			////// weighted aggeragted disparites by polygon
			std::vector<SpatialCoords> polygon_disp_values = c_sup_pts.aggregate_points_to_polygon<SpatialCoords, SpatialCoords>(support_points_disp, support_pts_weights, spatial_coords_weighted_mean_aggregator);
