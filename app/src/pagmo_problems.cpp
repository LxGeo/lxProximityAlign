	...
	namespace lxProximityAlign
	{
		...
		void pagmo_proximity_align_polygon(
			...
			*****/
			...
			std::vector<double> major_vector = { 0.0,0.0 };
			...
			for (auto distance_val_iter = neighbour_distance_band_values.rbegin(); distance_val_iter != neighbour_distance_band_values.rend(); ++distance_val_iter) {
				...
				for (size_t comp_idx = 0; comp_idx < PSW.n_components; ++comp_idx) {
					...
					std::vector<double> previous_step_disp_xy(2); previous_step_disp_xy[0]=previous_step_disp_x_value, previous_step_disp_xy[1]=previous_step_disp_y_value;
					...
					std::function<double(const std::vector<double>&)> bound_objective = [&RPR, &null_value, &linestring_fitness_evaluator, &component_exterior_edges](const std::vector<double>& disp_xy)->double {
						double total_fitness = 0;
						double total_length = 0;
						for (auto& c_exterior_edge : component_exterior_edges) {
							double c_edge_length = bg::length(c_exterior_edge);
							bg::strategy::transform::translate_transformer<double, 2, 2> trans_obj(disp_xy[0], disp_xy[1]);
					...
					std::function<double(const std::vector<double>&)> improved_f = [&bound_objective, &major_vector, &MAX_DISP](const std::vector<double>& pt) {

						auto cosine_similarity = [](const std::vector<double>& v1, const std::vector<double>& v2) {
						...
						double fitness_factor = bound_objective({ pt[0], pt[1] });
					...
					double previous_optimal_fitness = improved_f({ previous_step_disp_xy[0], previous_step_disp_xy[1] });
